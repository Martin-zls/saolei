        ...
        for(var j in table){
          if(table[i][j] == 0 ){
            ...
            if( i >= 1 && i <= imax+1 && j >= 1 && j <= jmax+1 ){
              a += table[i-1][j-1]==-1?1:0;
              //console.log(i,j,i*j,table[i-1][j-1]);
            }
            if( i >= 1 && i <= imax+1 && j >= 0 && j <= jmax ){
              a += table[i-1][j]==-1?1:0;
              //console.log(i,j,i*j,table[i-1][j]);
            }
            if( i >= 1 && i <= imax+1 && j >= -1 && j <= jmax-1 ){
              a += table[i-1][j+1]==-1?1:0;
              //console.log(i,j,i*j,table[i-1][j+1]);
            }
            if( i >= 0 && i <= imax && j >= 1 && j <= jmax+1 ){
              a += table[i][j-1]==-1?1:0;
              //console.log(i,j,i*j,table[i][j-1]);
            }
            if( i >= 0 && i <= imax && j >= -1 && j <= jmax-1 ){
              a += table[i][j+1]==-1?1:0;
              //console.log(i,j,i*j,table[i][j+1]);
      ...
      for( var i in table){
        for ( var j in table[i] ){
          if( table[i][j] > 10 && table[i][j] < 18){
            ...
            //htmlstr += '<div data-i="' + i + '" data-j=' + j + '>'+table[i][j]+'</div>';
          }else if( table[i][j] == 9 ){
            htmlstr += '<div class="lei" data-i="' + i + '" data-j=' + j + '>雷</div>';
          }else if(  table[i][j] > 30 && table[i][j] <= 49 ){
            //htmlstr += '<div class="leiclose" data-i="' + i + '" data-j=' + j + '>'+table[i][j]+'</div>';
            htmlstr += '<div class="leiclose" data-i="' + i + '" data-j=' + j + '>雷</div>';
          }else {
            htmlstr += '<div class="close" data-i="' + i + '" data-j=' + j + '></div>';
            //htmlstr += '<div class="close" data-i="' + i + '" data-j=' + j + '>'+table[i][j]+'</div>';
          }
    ...
    function shaolei(table){
      ...
      $('#wrapper').on('mousedown','.close',function(e){
        ...
        if( e.button == 0 ){
          ...
          table[i][j] = parseInt(table[i][j]) + 40;
          $(this).addClass('leiclose').removeClass('close').html('雷');
        }
      });
